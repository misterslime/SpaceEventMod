...
internal partial class FluidSimulation
{
    ...
    private float SmoothingKernelPoly6(float radius, float distance)
    {
        ...
        var value = radius * radius - distance * distance;
    ...
    private Vector3 SignedDistanceGradientSegment(Vector2 p, Vector2 a, Vector2 b, float r)
    {
        ...
        var h = MathHelper.Clamp(Vector2.Dot(pa, ba) / Vector2.Dot(ba, ba), 0.0f, 1.0f);
        var q = pa - h * ba;
        var d = q.Length();
        var g = q / d;
    ...
    {
        k *= 4.0f;
        var h = MathF.Max(k - MathF.Abs(a.X - b.Y), 0.0f) / (2.0f * k);
        var gradient = Vector2.Lerp(new(a.Y, a.Z), new(b.Y, b.Z), (a.Y < b.X) ? h : 1.0f - h);
