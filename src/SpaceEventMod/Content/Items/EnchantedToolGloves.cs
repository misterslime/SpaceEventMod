...
internal class EnchantedToolGlovesPlayer : ModPlayer
{
    ...
    public override void PreUpdateBuffs()
    {
        ...
        bool notValidTool = item.pick <= 0 && item.axe <= 0 && item.hammer <= 0 && !specialToolUsageSettings.IsAValidTool;
        ...
        if (Player.toolTime > 0)
        {
            ...
            {
                float smokeSize = Main.rand.NextFloat(0.9f, 1);
                Vector2 gushDirection = Vector2.UnitX * -Player.direction;
                gushDirection = gushDirection.RotatedByRandom(0.6f);

                Vector2 mistCenter = Player.GetFrontHandPosition(Player.compositeFrontArm.stretch, Player.compositeFrontArm.rotation);
                mistCenter -= Player.MountedCenter;
                mistCenter = Player.MountedCenter - mistCenter;

                Vector2 velocity = gushDirection * Main.rand.NextFloat(0.45f, 0.9f);

                Dust mist = Dust.NewDustPerfect(mistCenter, ModContent.DustType<Mist>(), velocity);
