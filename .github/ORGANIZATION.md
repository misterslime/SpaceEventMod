...
Example: a source file at `Content/CellularGrowth` should have a namespace of `Content.CellularGrowth`, however a source file located at `Content/CellularGrowth/_NPCs` would still have the previous namespace.

**Limit namespace element count to 3** - `.NPCs.Dropling` in `SpaceEventMod.Content.CellularGrowth.NPCs.Dropling` is illegal and I will cry.

## Directories

`Assets`: Contains assets, such as sounds, images, shaders, 3d models, structures, localization, etc.

`Common`: Contains infrastructure, such as graphics and physics systems, commands, gameplay systems, mechanics, worldgen, etc. "Engine-ish" functionality that will power most of your mod. Rule of thumb is this is for behavior implementations that are reused across content.
...
`_Utilities`: Contains utilities split by what references they utilize (/_Xna/, /_Terraria/, /_TModLoader/).
...
<summary>Various subtype examples</summary>
